			...
			create: function (opt) {
				...
				fn = {
					_normal: function () {
						...
						options.popObj.find("div.popup-box").css({ "left": ((document.body.clientWidth - options.popObj.find("div.popup-box").outerWidth()) / 2) + "px", "top": ((document.body.clientHeight - options.popObj.find("div.popup-box").outerHeight()) / 2) + "px" });
					},
					_ajax: function () {	
						$.ajax({
							type: "POST",
							...
							beforeSend: function () {
								if(options.height=="auto"){
									options.popObj.find("div.popup-content").css("height", "50");
								}
				...
				//拖拽操作
				var _move = false;
				var _x=0, _y=0, moveObj = options.popObj.find("div.popup-box");//定义移动后坐标点  
