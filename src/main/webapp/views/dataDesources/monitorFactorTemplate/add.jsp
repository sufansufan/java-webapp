...
<div class="layui-form" action="" style="padding: 20px">
	...
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<select multiple="" id="factorTag" name="factorTag"  xm-select="select7_7" xm-select-search="" xm-select-show-count="3" xm-select-height="30px" xm-select-width="200px">
	...
	<div class="layui-form-item">
		<label class="layui-form-label"><span style="color:red">*</span>设备名</label>
		<div class="layui-input-block">
			<select name="machineId" lay-verify="required">
				<option value="">请选择</option>
				<c:forEach var="item" items="${machineNameList}" varStatus="vs">
					<option value="${item.id}" ${model.machineType==item.machineType?'selected="selected"':''}>${item.machineName}</option>
				</c:forEach>
			</select>
		</div>
	</div>
  ...
  <div class="layui-form-item">
	...
	<div class="layui-form-item">
		...
		<div class="layui-input-block">
			<input type="text" name="lowerLimitText" placeholder="[监控因子名]-异常" lay-verify="textMaxlength" class="layui-input">
		</div>
	</div>
	<div class="layui-form-item">
		<label class="layui-form-label">高值报警信息</label>
		<div class="layui-input-block">
			<input type="text" name="upperLimitText" placeholder="[监控因子名]-异常" lay-verify="textMaxlength" class="layui-input">
		</div>
	</div>
...
$(function(){
	...
	form.on('submit(monitorFactorButton)', function(data) {
		monitorFactorTemplateAddEditSave(data.field,1);
		return false;
	});
	//监听类型改变
